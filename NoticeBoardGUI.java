...
public class NoticeBoardGUI {
    ...
    class NoticeBoard {
        ...
        void loadFromFile() {
            ...
            try (BufferedReader br = new BufferedReader(new FileReader(file))) {
                String line;
                java.util.List<Notice> loaded = new java.util.ArrayList<>();
                while ((line = br.readLine()) != null) {
                    Notice n = Notice.fromFileLine(line);
                    if (n != null) loaded.add(n);
                }
                // Enqueue in file order, push in reverse so stack top = most recent
                for (Notice n : loaded) {
                    queue.enqueue(n);
                ...
                // Push in reverse so last notice in file = top of stack (most recent)
                for (int i = loaded.size() - 1; i >= 0; i--) {
                    stack.push(loaded.get(i));
                }
            } catch (Exception e) {
    ...
    void adminPanel() {
        ...
        addBtn.addActionListener(e -> {
            String t = title.getText().trim().replace("|", "-");
            String m = msg.getText().trim().replace("|", "-");
            String k = keyword.getText().trim().replace("|", "-");
            String d = dateField.getText().trim().replace("|", "-");
            String c = (String) cat.getSelectedItem();

            if (t.isEmpty() || m.isEmpty() || k.isEmpty() || d.isEmpty()) {
